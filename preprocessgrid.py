...
def calculate_cell_face_areas(grid_coords:pd.DataFrame, grid_size:list):
    ...
    eta_x = np.zeros(xc_size*yc_size)
    eta_y = np.zeros(xc_size*yc_size)
    xi_x = np.zeros(xc_size*yc_size)
    xi_y = np.zeros(xc_size*yc_size)
    ...
    for iy in range(yc_size-2):
        ...
        for ix in range(xc_size-2):
            ...
            # TODO make sure to account for halo cells
            # (i.e. the '-2' part in currIdx) automatically
            currIdx = (iy*(xc_size-2)) + ix 
            xc[currIdx] = xcrd
            yc[currIdx] = ycrd
    ...
    area['eta_x'] = pd.Series(eta_x)
    area['eta_y'] = pd.Series(eta_y)
    area['xi_x'] = pd.Series(xi_x)
    area['xi_y'] = pd.Series(xi_y)

    return area, xc_size-2, yc_size-2

def plot_contour(grid_coords:pd.DataFrame, show_plot:bool=True, save_plot:bool=True, 
                 save_plot_as:str='plots', hue='src', cmap='viridis', 
                 colorbar_label='Cell Volume', coord_loc:list=[0,0],
                 coord_loc_2:list=[-0.495,0]):
    """
